...
const Footer = () => {
  const searchUrl = ({ search, text }) => {
    let url;

    if (search === 'IEEE')
      url = `https://ieeexplore.ieee.org/search/searchresult.jsp?newsearch=true&queryText=${text}`;
    else
      url = `https://scholar.google.com.vn/scholar?q=${text}`;
              ...
              <div className="widget-tags">
                <ul>
                  {keywordData && keywordData.map((keyword, idx) => (
                    <li key={`keyword-${idx}`}>
                      <a target="_blank" rel="noreferrer" href={searchUrl(keyword.search, keyword.text)}>
                        {keyword.name}
                      </a>
                    </li>
                  ))}
