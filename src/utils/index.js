...
const containsObject = (obj, list) => {
  for (let i = 0; i < list.length; i++) {
    if (list[i].slug === obj.slug) return i;
  }
...
const shortTitle = (title, maxlength = 40) => {
  ...
  for (idx; idx < words.length; idx++) {
    if (words.slice(0, idx).join(' ').length > maxlength) break;
  }
...
const keywordUrl = (keyword, search = 'IEE') => {
  ...
  else if (search === 'Springer') url = `https://link.springer.com/search?query=${keyword}`;
